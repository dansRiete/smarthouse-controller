...
public interface IndicationRepository extends JpaRepository<Indication, Integer> {
    ...
    @Query(value = "(select date_trunc('hour', received_local) +\n" +
            "        cast((FLOOR(DATE_PART('minute', received_local) / 5) * 5 || 'minutes') as interval) as msg_received,\n"
            +
            "        in_out,\n" +
            "        indication_place,\n" +
            "        'MINUTELY'                                        as period,\n" +
            ...
            "   AND date_trunc('day', received_local) = date_trunc('day', now() at time zone 'utc')\n" +
            ...
            "    AND received_local <= now() at time zone 'utc'\n" +
            " group by msg_received, in_out, indication_place\n" +
            " union all\n" +
            " select date_trunc('hour', received_local) +\n" +
            "        cast((FLOOR(DATE_PART('minute', received_local) / 60) * 60 || 'minutes') as interval) as msg_received,\n"
            +
            "        in_out,\n" +
            "        indication_place,\n" +
            "        'HOURLY'                                                         as period,\n" +
            ...
            "        from (select date_trunc('hour', received_local) +\n" +
            "                     cast((FLOOR(DATE_PART('minute', received_local) / 5) * 5 || 'minutes') as interval) as msg_received\n"
            +
            "              from main.indication\n" +
            "                       left join main.air a on a.id = indication.air_id\n" +
