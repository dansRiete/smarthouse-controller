...
select
    date_trunc('hour', received) + (FLOOR(DATE_PART('minute', received) / :interval_min ) * :interval_min || 'minutes')::interval as msg_received,
    concat(heat_indication.indication_place, '/', (CAST(count(*) as text))),
...
-- where indication_place = 'BALCONY'
-- where indication_place = 'TERRACE_ROOF'
-- where indication_place = 'CHERNIVTSI_AIRPORT'
-- where indication_place = 'LIVING_ROOM'
group by heat_indication.indication_place,
         date_trunc('hour', received) + (FLOOR(DATE_PART('minute', received) / :interval_min ) * :interval_min || 'minutes')::interval
order by msg_received DESC;

-- Aggregated AIR QUALITY DATA
select
        date_trunc('hour', received) + (FLOOR(DATE_PART('minute', received) / :interval_min ) * :interval_min || 'minutes')::interval as msg_received,
        concat(air_quality_indication.indication_place, '/', (CAST(count(*) as text))),
...
from main.house_state
         inner join main.air_quality_indication_indication on house_state.id = air_quality_indication.house_state_id
-- where indication_place = 'BALCONY'
-- where indication_place = 'TERRACE_ROOF'
-- where indication_place = 'CHERNIVTSI_AIRPORT'
-- where indication_place = 'LIVING_ROOM'
where indication_place = 'CHILDRENS' /*AND*/
--         received > '2020-11-10 11:10:00'
group by air_quality_indication.indication_place,
         date_trunc('hour', received) + (FLOOR(DATE_PART('minute', received) / :interval_min ) * :interval_min || 'minutes')::interval
order by msg_received DESC;
...
select
        date_trunc('hour', received) + (FLOOR(DATE_PART('minute', received) / :interval_min ) * :interval_min || 'minutes')::interval as msg_received,
        concat(wind_indication.indication_place, '/', (CAST(count(*) as text))),
...
from main.house_state
         inner join main.air_wind_indication_indication on house_state.id = wind_indication.house_state_id
-- where indication_place = 'BALCONY'
-- where indication_place = 'TERRACE_ROOF'
-- where indication_place = 'CHERNIVTSI_AIRPORT'
-- where indication_place = 'LIVING_ROOM'
group by date_trunc('hour', received) + (FLOOR(DATE_PART('minute', received) / :interval_min ) * :interval_min || 'minutes')::interval
order by msg_received DESC;

-- Main indications
select house_state.received, heat_indication.indication_place, heat_indication.temp_celsius as temp,
...
from main.house_state
         ...
         left join main.air_quality_indication_indication on house_state.id = air_quality_indication.house_state_id
         left join main.air_wind_indication_indication on house_state.id = wind_indication.house_state_id
order by received desc LIMIT 3000;